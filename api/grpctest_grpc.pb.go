...
type EventClient interface {
	PublishEvent(ctx context.Context, opts ...grpc.CallOption) (Event_PublishEventClient, error)
}
...
func (c *eventClient) PublishEvent(ctx context.Context, opts ...grpc.CallOption) (Event_PublishEventClient, error) {
	stream, err := c.cc.NewStream(ctx, &Event_ServiceDesc.Streams[0], "/ingressv1.Event/PublishEvent", opts...)
	if err != nil {
		return nil, err
	}
	x := &eventPublishEventClient{stream}
	return x, nil
}

type Event_PublishEventClient interface {
	Send(*EventMessage) error
	CloseAndRecv() (*EventReturn, error)
	grpc.ClientStream
}

type eventPublishEventClient struct {
	grpc.ClientStream
}

func (x *eventPublishEventClient) Send(m *EventMessage) error {
	return x.ClientStream.SendMsg(m)
}

func (x *eventPublishEventClient) CloseAndRecv() (*EventReturn, error) {
	if err := x.ClientStream.CloseSend(); err != nil {
		return nil, err
	}
	m := new(EventReturn)
...
type EventServer interface {
	PublishEvent(Event_PublishEventServer) error
...
func (UnimplementedEventServer) PublishEvent(Event_PublishEventServer) error {
...
func _Event_PublishEvent_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(EventServer).PublishEvent(&eventPublishEventServer{stream})
}

type Event_PublishEventServer interface {
	SendAndClose(*EventReturn) error
	Recv() (*EventMessage, error)
	grpc.ServerStream
}

type eventPublishEventServer struct {
	grpc.ServerStream
}

func (x *eventPublishEventServer) SendAndClose(m *EventReturn) error {
	return x.ServerStream.SendMsg(m)
}

func (x *eventPublishEventServer) Recv() (*EventMessage, error) {
	m := new(EventMessage)
